...
score: 9.0
evidence: 直接解决模型编辑中的词汇偏差问题
tldr: 现有基于适配器的模型编辑方法严重受词汇偏差影响，导致对不相关提示误应用编辑。本文提出学习解耦表示空间，通过保持不相关提示的距离来精确定位编辑，实验证明在多种编辑任务上显著提升准确性和鲁棒性。
...
motivation: 当前基于适配器的模型编辑方法存在词汇偏差，导致编辑被错误应用到有重叠词汇的不相关提示上。
method: 学习一个解耦的表示空间，在不相关提示间保持距离，同时保留相近语义触发编辑。
result: 在多个编辑基准上，该方法显著减少了误编辑，提高了编辑精确度。
conclusion: 为模型编辑提供了一种原理性方法，以缓解词汇偏差，提升编辑可靠性。
---
