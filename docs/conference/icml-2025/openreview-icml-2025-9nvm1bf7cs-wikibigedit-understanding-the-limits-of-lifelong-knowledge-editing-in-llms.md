...
title_zh: WikiBigEdit：理解大语言模型中终身知识编辑的极限
...
tldr: 大语言模型的终身知识更新至关重要，但现有知识编辑方法仅在小型或合成基准上测试。本文推出WikiBigEdit，一个大规模真实维基数据编辑基准，包含超过50万问答对，并设计了自动化持续扩展的评估流程。利用该基准研究现有知识编辑技术的终身编辑能力，揭示了当前方法的局限。该基准为知识编辑研究提供了更贴近实际的标准测试平台。
...
motivation: 现有知识编辑方法缺乏大规模真实世界终身编辑基准。
method: 构建基于真实维基数据编辑的规模可扩展基准，包含50万问答对。
result: 研究现有知识编辑技术在终身场景下的表现，发现其性能下降。
conclusion: WikiBigEdit为推动终身知识编辑研究提供了重要资源。
---
