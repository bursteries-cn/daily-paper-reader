...
score: 9.0
evidence: 利用机械可解释性进行知识编辑和遗忘的方法
tldr: 该论文指出当前知识编辑和遗忘方法缺乏精确性，提出利用机械可解释性定位模型组件（电路）以提高编辑和遗忘的鲁棒性。实验表明，不同定位方法对编辑鲁棒性影响显著，区分了基于输出保留和基于高层机制的定位。该工作为模型编辑提供了更精细、更鲁棒的方案。
...
motivation: 现有知识编辑和遗忘方法不够精确，容易影响模型通用性能。
method: 利用机械可解释性定位与特定机制相关的模型组件，仅在这些组件上进行编辑或遗忘。
result: 不同定位方法导致的编辑鲁棒性差异显著，基于高层机制的定位更鲁棒。
conclusion: 机械可解释性可以提升知识编辑和遗忘的精确性和鲁棒性。
---
