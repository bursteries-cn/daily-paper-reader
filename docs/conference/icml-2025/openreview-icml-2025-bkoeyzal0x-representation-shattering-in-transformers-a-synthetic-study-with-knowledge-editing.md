...
title_zh: Transformer中的表示碎片化：基于知识编辑的合成研究
...
score: 9.0
evidence: 知识编辑负面影响的合成研究
tldr: 知识编辑在更新事实时可能损害模型整体推理能力，但原因不明。本文在合成知识图上训练Transformer，可控制编辑的结构化影响。实验揭示了编辑导致表示碎片化，破坏实体间关系，从而引起广泛性能下降。该工作从机制上解释了编辑的副作用，为设计更安全的编辑方法提供了理论指导。
...
motivation: 知识编辑会损害模型推理能力，但机制尚不清楚。
method: 在合成结构化的知识图上训练Transformer，分析编辑对表示的影响。
result: 编辑导致表示碎片化，破坏关系结构，造成性能下降。
conclusion: 揭示了知识编辑副作用的内在机制，为安全编辑方法奠定基础。
---
